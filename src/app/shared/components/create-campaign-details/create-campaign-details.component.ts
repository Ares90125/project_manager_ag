...
export class CreateCampaignDetailsComponent extends BaseComponent implements OnInit, OnDestroy {
	...
	productPurchaseRequired = {value: 'Product purchase required', isSelected: false};
	assetsTextRequired = {value: 'Text required', isSelected: false};
	assetsVideoRequired = {value: 'Video required', isSelected: false};
	cmcTypeList = [
	...
	campaignBriefImageUpload(image: any) {
		const element = this.quillFileRef.nativeElement as HTMLInputElement;
		element.value = null;
	...
	buildForm() {
		this.campaignDetailsForm = this.formBuilder.group({
			...
			assetsImagesRequired: [
				null,
				{
					validators: [Validators.pattern(/^\d*[0-9]+\d*$/), Validators.max(10)]
				}
			]
		});
	...
	patchFormValue(campaign: CampaignModelv3) {
		this.campaignDetailsForm.patchValue({
			...
			productPurchaseInfo: campaign.productPurchaseInfo,
			trainingLinkMessage: campaign.trainingLinkMessage,
			assetsImagesRequired: campaign.assetsImagesRequired
		});

		this.productPurchaseRequired.isSelected = campaign['productPurchaseRequired'];
		this.assetsTextRequired.isSelected = campaign['assetsTextRequired'];
		this.assetsVideoRequired.isSelected = campaign['assetsVideoRequired'];
	...
	submitDataToParent() {
		...
		campaignDetails['productPurchaseRequired'] = this.productPurchaseRequired.isSelected;
		campaignDetails['assetsTextRequired'] = this.assetsTextRequired.isSelected;
		campaignDetails['assetsVideoRequired'] = this.assetsVideoRequired.isSelected;
	...
	selectProductPurchaseRequired(isChecked: boolean) {
		this.productPurchaseRequired.isSelected = isChecked;
		if (!isChecked) {
			this.campaignDetailsForm.get('productPurchaseInfo').setValue('');
		}
		this.submitDataToParent();
	}

	selectAssetsTextRequired(isChecked: boolean) {
		this.assetsTextRequired.isSelected = isChecked;
		this.submitDataToParent();
	}

	selectAssetsVideoRequired(isChecked: boolean) {
		this.assetsVideoRequired.isSelected = isChecked;
		this.submitDataToParent();
	}
	...
	openCampaignBriefPage() {
		window.open(`${environment.baseUrl}public/campaign-brief/${this.campaign.campaignId}`);
	}
