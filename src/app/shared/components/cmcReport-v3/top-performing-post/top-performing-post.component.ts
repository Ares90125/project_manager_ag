...
export interface TopPerformingPost extends CampaignPost, INewScreenshotAdded {
	id: string;
...
export interface BrandLevelFilter {
	showAdminPosts: boolean;
	showUGCPost: boolean;
	from: number;
	end: number;
	limit: number;
...
export interface PopupLevelFilter extends BrandLevelFilter {
	showVisibleToBrand: Boolean;
	showIsSystemGeenrated: Boolean;
	showManuallyUploaded: Boolean;
	showPost: Boolean;
	showComment: Boolean;
	searchText?: string;
}
...
@Component({
	selector: 'app-top-performing-post',
	templateUrl: './top-performing-post.component.html',
	styleUrls: ['./top-performing-post.component.scss']
	// changeDetection: ChangeDetectionStrategy.Default
})
export class TopPerformingPostComponent extends BaseComponent implements OnInit, OnChanges, OnDestroy {
	@Input()
	...
	brandLevelFilters: BrandLevelFilter = {
		showAdminPosts: undefined,
		showUGCPost: undefined,
		from: 0,
		end: 10,
		limit: 10
	};
	...
	EditPopupLevelFilters: PopupLevelFilter = {
		...
		startDate: null,
		endDate: null,
		showVisibleToBrand: null,
		showComment: null,
		showManuallyUploaded: null,
		showPost: null,
		showIsSystemGeenrated: null,
		searchText: null
	};
	...
	ngOnInit(): void {
		try {
			...
			this.listenToTopPerformaingPostScroll();
	...
	ngOnChanges(change: SimpleChanges) {
		...
		if (change.isBrandLoggedIn.currentValue) {
			this.listenToTopPerformaingPostEditScroll();
		}
	}
		...
		if (!this.screenshotsObservables[id]) {
			const observable = defer(() =>
				...
				map(response => {
					...
					this.screenshotProcessed[id] = true;
					return response;
				}),
				retryWhen(errors => {
					return errors.pipe(
	...
	private listenToTopPerformaingPostScroll() {
		if (this.baseLevelScrollSubscription) return;
	...
	private listenToTopPerformaingPostEditScroll() {
		if (this.editPopupScrollSubscription || !this.brandViewPopupImageContainer?.nativeElement) return;
	...
	private setBaseLevelPosts(list: TopPerformingPost[]) {
		this.baseLevelFilteredPost = list
			?.filter(post => {
