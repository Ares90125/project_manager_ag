...
export class ConversationVolumeReportModel {
	...
	getHighChartsModel() {
		...
		this.reportData.chartOptions.yAxis.title.text = 'Conversation Volume';
		...
		const total = _.reduce(
			...
			(sum, datapoint) =>
				sum +
				Number(
					_.reduce(datapoint.ys, (sumYs, ys) => sumYs + (ys.label === 'Conversation Volume' ? Number(ys.value) : 0), 0)
				),
			0
		);
